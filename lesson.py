...
url = "https://grantist.com/countries/usa/"
#
headers = {
...
soup = BeautifulSoup(src, "lxml")
konkursy = soup.find_all("article", class_="contests-vse-konkursy")
stajirovky = soup.find_all("article", class_="internships-vse-stazhirovki")
obucheniye = soup.find_all("article", class_="obuchenie-bachelor")
# print(stajirovky,konkursy)

projects_urls = []
for article in konkursy:
    konkursy_url = article.find("h2", class_="entry-title").find("a").get("href")
    # print(konkursy_url)

for article in stajirovky:
    stajirovky_url = article.find("h2", class_="entry-title").find("a").get("href")
    # print(stajirovky_url)

for article in obucheniye:
    obucheniye_url = article.find("h2", class_="entry-title").find("a").get("href")
    # print(obucheniye_url)
    projects_urls.append(konkursy_url)
    projects_urls.append(stajirovky_url)
    projects_urls.append(obucheniye_url)

# print(projects_urls)
projects_data_list = []
for konkursy_url in projects_urls:
    req = requests.get(konkursy_url, headers)
    project_name = konkursy_url.split("/")[-2]
    ...
    soup = BeautifulSoup(src, "lxml")
    project_data = soup.find("div", class_="site-content")
    # print(project_data)

    try:
       project_name = project_data.find("header", class_="entry-header clearfix").find('h1').text
       # print(project_name)
    except Exception:
        project_name = "No project name"

    try:
       prochee = project_data.find("div", class_="description").find('p').text
       # print(prochee)
    except Exception:
        prochee = "No project name"

    try:
       treb = project_data.find("div", class_="requirements").find('ul').text
       # print(treb)
    except Exception:
        treb = "No project name"

    projects_data_list.append(
        {
            "название": project_name,
            "описание": prochee.strip(),
            "треования": treb.strip()
        }
    )
